...
class events(commands.Cog):
    ...
    async def get_webhook(self, channel):
        ...
        hook = await channel.create_webhook(name="DaPanda ModMail")
        return hook
    ...
    async def modmail(self, message):
        ...
        category = self.bot.get_guild(875005644594372638).get_channel(879214679711420438)
        ...
        try:
            ...
            await message.add_reaction('<:tickYes:885222891883470879>')
        except: return await message.add_reaction('<:tickNo:885222934036226068>')

    @commands.Cog.listener('on_message')
    async def modmail_reply(self, message):
        if not message.guild: return
        if message.author.bot: return
        if message.channel.category_id != 879214679711420438: return
